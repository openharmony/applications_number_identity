...
#ifndef DOWNLOAD_FILE_DB_ABILITY_H
#define DOWNLOAD_FILE_DB_ABILITY_H
...
class DownloadFileAbility : public DataShare::DataShareExtAbility {
public:
    DownloadFileAbility(std::shared_ptr<DataShareExtAbilityContext> parentContext);
    virtual ~DownloadFileAbility() override;
    static std::shared_ptr<DownloadFileAbility> Create(std::shared_ptr<DataShareExtAbilityContext> parentContext);
...
private:
    void DealTaskTypeOfWorkSheduler(const DataShare::DataShareValuesBucket &value);
    std::shared_ptr<DataShareExtAbilityContext> parentContext_;
    std::mutex lock_;
};
} // namespace Telephony
} // namespace OHOS
#endif // DOWNLOAD_FILE_DB_ABILITY_H
